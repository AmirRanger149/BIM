<template>
  <div class="admin-hero-sliders admin-page">
    <div class="admin-section-header">
      <div>
        <div class="eyebrow">🎬 اسلایدرهای صفحه اصلی</div>
        <h2>مدیریت Hero Sliders</h2>
        <p class="muted">مدیریت تصاویری که در قسمت بالای صفحه اصلی (Hero) نمایش می‌یابند</p>
        <div class="meta-chips">
          <span class="chip">{{ heroSliders.length }} اسلایدر</span>
      ...
      <div class="modal-card admin-modal">
        ...
        <form @submit.prevent="submitForm" class="slider-form">
          <div class="form-group">
            ...
            <input v-model="formData.name" type="text" required placeholder="مثال: hero-1, header-main" />
            <small class="form-hint">یک نام یکتا برای اسلایدر وارد کنید</small>
          </div>

          <div class="form-group">
            <label>توضیح (اختیاری)</label>
            <textarea v-model="formData.description" rows="2" placeholder="توضیح اسلایدر..."></textarea>
          </div>
    ...
    <!-- لیست اسلایدرهای Hero -->
    <div class="panel">
      <div v-if="loading" class="loading">درحال بارگذاری...</div>
      <div v-else-if="heroSliders.length === 0" class="empty">
        <p>هیچ اسلایدری برای Hero یافت نشد</p>
        <p class="sub-text">برای شروع یک اسلایدر جدید ایجاد کنید</p>
      </div>
      <div v-else class="sliders-grid">
        <div v-for="slider in heroSliders" :key="slider.id" class="slider-card glass-card">
...
const totalImages = computed(() => heroSliders.value.reduce((sum, s) => sum + (s.images?.length || 0), 0))

const loadHeroSliders = async () => {
  loading.value = true
  try {
    const sliders = await adminService.getSliders()
    // فیلتر تنها اسلایدرهای Hero
    heroSliders.value = sliders.filter(s => s.name && /hero|header|home/i.test(s.name))
  } catch (err) {
...
const submitForm = async () => {
  if (!formData.value.name.trim()) {
    notifyError('نام اسلایدر الزامی است')
    return
  }
  ...
  try {
    if (editingId.value) {
      ...
      notifySuccess('اسلایدر Hero به‌روزرسانی شد')
    } else {
      await adminService.createSlider(formData.value)
      notifySuccess('اسلایدر Hero ایجاد شد')
    }
    closeForm()
...
const deleteSlider = async (id) => {
  if (!confirm('آیا از حذف این اسلایدر Hero مطمئن هستید؟')) return
  try {
    await adminService.deleteSlider(id)
    notifySuccess('اسلایدر Hero حذف شد')
    await loadHeroSliders()
  } catch (err) {
    notifyError(err.response?.data?.detail || 'عملیات ناموفق')
  }
...
<style scoped>
.admin-hero-sliders { display: flex; flex-direction: column; gap: 1.5rem; }
.header-actions { display: flex; gap: 0.75rem; flex-wrap: wrap; }
.slider-form { padding: 1.25rem 1.5rem; display: flex; flex-direction: column; gap: 1rem; }
.form-hint { display: block; margin-top: 0.35rem; font-size: 0.8rem; color: #6b7280; font-style: italic; }
...
.images-preview h4 { margin: 0 0 0.75rem 0; color: #1f2937; font-weight: 700; }
